...
export async function getServerSideProps(context) {
    ...
    const url = `http://localhost:4000/series/page/${context.query.page ? context.query.page : "1"}${
...
export default function SeriesPage({ series, seriesCount }) {
    ...
    const changePage = ({ selected }) => {
        ...
        if (router.query.sortBy && router.query.ascOrDesc)
            router.push(
                `/series?page=${selected + 1}&sortBy=${router.query.sortBy}&ascOrDesc=${router.query.ascOrDesc}`
            );
    };
