...
export async function getServerSideProps(context) {
    const url = `http://localhost:4000/movies/page/${context.query.page ? context.query.page : "1"}${
    ...
    const moviesCount = res3.data;
    const moviePayload = {
        title: context.query.search ? context.query.search : "iewhfewoifhewfioeoijfewoifjewijfijewof",
...
export default function Home({ movies, latestMovies, moviesCount, searchedMovies }) {
    ...
    if (router.query.search) pageCount = Math.ceil(searchedMovies.count / itemsPerPage);
    ...
    const changePage = ({ selected }) => {
        ...
        if (router.query.sortBy && router.query.ascOrDesc)
            router.push(`/?page=${selected + 1}&sortBy=${router.query.sortBy}&ascOrDesc=${router.query.ascOrDesc}`);
    };
    ...
    return (
        <>
            <div className={styles["home-wrapper-menus"]}>
                <Header />
                <div className={styles["home-ribbon-1"]}></div>
              ...
              Total movies: {moviesCountSearch}{" "}
                    ...
                    <span className={styles["movie-count-span"]}>Total movies: {moviesCount?.count}</span>
                        ...
                        <div className={styles["image-ribbon-3-wrapper"]}>
                            {latestMovies?.map(latestMovie => (
                                ...
                                >
                                    ...
                                    <span className={styles["movie-title"]}>{latestMovie.title}</span>
                                    ...
                                    <span className={styles["imdb-span"]}>
                                        {latestMovie.ratingImdb !== 0 ? "Imdb: " + latestMovie.ratingImdb : null}
                                    </span>
                                </div>
                            ))}
                        </div>
